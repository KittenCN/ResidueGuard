...
if [[ "$MODE" != --build-only ]]; then /usr/bin/pkill -x ResidueGuard 2>/dev/null || true; fi
/usr/bin/xcodebuild -project "$ROOT_DIR/ResidueGuard.xcodeproj" -scheme ResidueGuard -configuration Debug -destination 'platform=macOS' -derivedDataPath "$ROOT_DIR/.build-xcode" CODE_SIGN_IDENTITY=- CODE_SIGNING_ALLOWED=YES build
