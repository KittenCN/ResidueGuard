...
public final class VerifiedBackup {
    ...
    fileprivate func readSource(_ name: String) throws -> (Data, SourceFingerprint) {
...
extension VerifiedBackup {
    ...
    public static func runISO01VirtualMachineProbe() throws -> String {
        ...
        let receipt = try store.prepare(name: name, expected: fingerprint, planID: UUID())
        try store.verify(receipt)
        guard try store.inspect(name: name) == fingerprint else { throw BackupFailure.changed }
