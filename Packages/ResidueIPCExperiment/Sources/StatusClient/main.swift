...
func request(_ frame: Data, connection: NSXPCConnection, session: Int32) -> String {
    let box = ReplyBox(), peer = ConnectionBox(connection)
    guard let proxy = connection.remoteObjectProxyWithErrorHandler({ _ in box.finish("transportRejected") }) as? RGStatusWire else { return "transportRejected" }
    proxy.inspectFrame(frame) { data in
        guard StatusLabPolicy.acceptsPeer(actualUID: peer.value.effectiveUserIdentifier,
            actualSession: peer.value.auditSessionIdentifier, currentUID: getuid(), currentSession: session),
              let reply = try? StatusLabReply.decode(data) else { box.finish("peerRejected"); return }
...
do {
    ...
    if configuration.scenario == .messageLimit {
        for _ in 0..<9 {
            let outcome = request(frame, connection: connection, session: session); outcomes.append(outcome)
        ...
        for index in 0..<5 {
            ...
            if index == 0 { candidate = connection }
            else {
                ...
                candidate.activate(); retained.append(candidate)
            }
            let outcome = request(frame, connection: candidate, session: session); outcomes.append(outcome)
            if outcome != "statusOnly" { break }
        }
        withExtendedLifetime(retained) {}
    } else if configuration.scenario == .invalidatedConnection {
        outcomes.append(request(frame, connection: connection, session: session))
        if outcomes.last == "statusOnly" {
            connection.invalidate()
            outcomes.append(request(frame, connection: connection, session: session))
        }
    } else { outcomes.append(request(frame, connection: connection, session: session)) }
